...
import type { Card, GameAction } from "@grandstand/shared";
import type { EnginePlayerView, GameEngine } from "../src/game-engine-adapter.js";
...
const fakeEngine: GameEngine = {
  createGame(playerIds) {
    const hands = Object.fromEntries(
      playerIds.map((playerId, index) => [
        playerId,
        [{ id: randomUUID(), top: index + 1, bottom: 10 - index, suit: "coral" as const }],
      ]),
    );
    const activePlayerId = playerIds[0];
    if (!activePlayerId) throw new Error("Players required");
    return { playerIds, hands, activePlayerId, phase: "orientation" } satisfies FakeState;
  },
  applyAction(state, _playerId, action) {
    const current = asFakeState(state);
    if (action.type === "game:choose-orientation") return { ...current, phase: "playing" };
    return current;
  },
  getPlayerView(state, playerId): EnginePlayerView {
    ...
    return {
      ...
      availableActions: {
        show: { enabled: false, disabledReason: "not-active-player", ranges: [] },
        scout: {
          enabled: false,
...
describe("RoomService", () => {
  it("creates secure sessions and emits only the viewer's private hand", () => {
    ...
    service.perform(created.sessionToken, { actionId: randomUUID(), type: "game:start" });
  ...
  it("deduplicates repeated action IDs and rejects conflicting reuse", () => {
    ...
    expect(service.perform(created.sessionToken, action).ack.duplicate).toBe(false);
    expect(service.perform(created.sessionToken, action).ack.duplicate).toBe(true);
  ...
  it("retains replay protection well beyond the former tiny cache window", () => {
    ...
    expect(service.perform(created.sessionToken, first).ack.duplicate).toBe(true);
  });
  ...
  it("converts an active leaver to a bot and drains through its turn", () => {
    ...
    const drainingEngine: GameEngine = {
      ...
      chooseBotAction() {
        return { actionId: randomUUID(), type: "game:show", cardIds: [randomUUID()] };
      },
    };
    const service = new RoomService(new InMemoryRoomRepository(), drainingEngine);
    ...
    host.room!.engineState = { ...(host.room!.engineState as FakeState), phase: "playing" };

    service.perform(host.sessionToken, { actionId: randomUUID(), type: "room:leave" });
    expect(host.player!.isBot).toBe(true);
    expect(botActions).toBe(1);
    expect((host.room!.engineState as FakeState).activePlayerId).toBe(guest.player!.id);
  });
...
function asFakeState(state: unknown): FakeState {
  if (typeof state !== "object" || state === null || !("playerIds" in state)) throw new Error("Bad fake state");
