...
describe("LobbyScreen experience polish", () => {
  it("confirms before the host leaves", () => {
    ...
    expect(screen.getByRole("dialog", { name: "Leave this lobby?" })).toBeInTheDocument();
    expect(screen.getByText(/Another connected player will become host/)).toBeInTheDocument();
  ...
  it("opens an accessible QR invite with only the canonical room URL", () => {
    window.history.pushState({}, "", "/table?sessionToken=do-not-share&room=OLD#private");
    const expectedUrl = new URL(`/?room=${lobbyState.roomCode}`, window.location.origin).toString();
    render(<LobbyScreen state={lobbyState} connected dispatch={vi.fn()} />);
    ...
    expect(screen.getByTitle(`Join Grandstand room ${lobbyState.roomCode}`)).toBeInTheDocument();
    expect(screen.getByTestId("invite-qr")).toHaveAttribute("data-invite-url", expectedUrl);
